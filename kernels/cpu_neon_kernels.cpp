...
inline float exact_sigmoid(float x) {
...
// ----------------------------------------------------------------------------
std::tuple<torch::Tensor, torch::Tensor> rmsnorm_forward_neon(
    torch::Tensor x,
    torch::Tensor weight,
    float eps
) {
...
// ----------------------------------------------------------------------------
std::tuple<torch::Tensor, torch::Tensor> rmsnorm_backward_neon(
    torch::Tensor grad_y,
    torch::Tensor x,
    torch::Tensor weight,
    torch::Tensor rsqrt_cache
) {
    ...
    at::parallel_for(0, num_rows, 0, [&](int64_t start, int64_t end) {
        for (int64_t r = start; r < end; ++r) {
            ...
            float scale_term = (inner_sum / static_cast<float>(dim)) * (rsqrt_val * rsqrt_val * rsqrt_val);
            float32x4_t rsqrt_vec = vdupq_n_f32(rsqrt_val);
            ...
            for (; i < dim; ++i) {
                row_gx[i] = (row_gy[i] * w_ptr[i] * rsqrt_val) - (row_x[i] * scale_term);
            }
        }
    });
...
// ----------------------------------------------------------------------------
torch::Tensor swiglu_forward_neon(
    torch::Tensor gate,
    torch::Tensor up
) {
...
// ----------------------------------------------------------------------------
std::tuple<torch::Tensor, torch::Tensor> swiglu_backward_neon(
    torch::Tensor grad_y,
    torch::Tensor gate,
    torch::Tensor up
) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("rmsnorm_forward_neon", &axiom::rmsnorm_forward_neon, "Axiom RMSNorm Forward (ARM NEON)");
    m.def("rmsnorm_backward_neon", &axiom::rmsnorm_backward_neon, "Axiom RMSNorm Backward (ARM NEON)");
    m.def("swiglu_forward_neon", &axiom::swiglu_forward_neon, "Axiom SwiGLU Forward (ARM NEON)");
    m.def("swiglu_backward_neon", &axiom::swiglu_backward_neon, "Axiom SwiGLU Backward (ARM NEON)");
}
