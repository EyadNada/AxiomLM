...
def main():
    ...
    config = ax.ModelConfig(
        arch="modern", block_size=512, vocab_size=50304, n_layer=6, n_head=8, n_embd=512
    )
    ...
    model.to(device)
    print(
        f"Model initialized with {sum(p.numel() for p in model.parameters())/1e6:.2f}M parameters on {device}."
    )
