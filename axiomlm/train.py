...
def train(
    ...
    if master_process:
        print(
            f"[AxiomLM Pretrain Engine] Running on device: {device} (World Size: {ddp_world_size})"
        )
    ...
    ), f"total_batch_size ({total_batch_size}) must be divisible by {B*T*ddp_world_size}"
    ...
    if resume:
        if os.path.isfile(resume):
            ...
            cleaned_sd = {
                ...
                for k, v in ckpt[model_key].items()
            }
            ...
            if master_process:
                print(
                    f"[AxiomLM Notice] Checkpoint {resume} not found. Starting from step 0."
                )
        ...
        for step in range(start_step, max_steps):
            ...
            if (eval_interval > 0 and step % eval_interval == 0) or last_step:
                ...
                with torch.no_grad():
                    ...
                    if master_process:
                        print(
                            f"\n[Val Eval @ Step {step:4d}] validation loss: {val_loss:.4f}",
                            flush=True,
                        )

            # 2. Live Validation Token Sampling
            if master_process and (
                (sample_interval > 0 and step % sample_interval == 0) or last_step
            ):
                raw_model = get_raw_model(model)
                samples = generate_with_cache(
                    raw_model,
                    enc,
                    device,
                ...
                print(
                    f"--- Live Generated Samples (Systems ML / Code) @ Step {step:4d} ---"
                )
            ...
            mfu_pct, achieved_tflops = calculate_mfu(
                model, tokens_per_sec, T, peak_tflops
            )
            ...
            if master_process:
                lr_str = (
                    f"muon_lr: {current_muon_lr:.4e} | adamw_lr: {current_adamw_lr:.4e}"
                    if optimizer_type == "muon"
                    else f"lr: {current_adamw_lr:.4e}"
                )
                print(
            ...
            if (
                ...
                and master_process
                and (
                    (save_interval > 0 and completed_step % save_interval == 0)
                    or last_step
                )
            ):
                ...
                print(
                    f"[AxiomLM] Saved checkpoint to {latest_p} (step {completed_step})"
                    + (f" (archived {os.path.basename(step_p)})" if step_p else ""),
                    flush=True,
                )
...
def main():
    ...
    parser.add_argument(
        "--arch", type=str, default="modern", choices=["classic", "modern"]
    )
    parser.add_argument(
        "--optimizer", type=str, default="muon", choices=["adamw", "muon"]
    )
    ...
    parser.add_argument(
        "--resume", nargs="?", const="checkpoints/model_latest.pt", default=None
    )
    ...
    if args.benchmark:
        ...
        benchmark_generation_speed(
            bm_model, enc, device, prompt=args.sample_prompt, max_length=100
        )
    else:
